...
let
  ...
  baseShadow = mkShadow {
    ...
    spread = 0;
    y = 1;
  }; # MD3 L1 — resting tiled windows + bar
  activeShadow = mkShadow {
    softness = 8;
    spread = 1;
    y = 3;
  }; # MD3 L3 — active / raised window
  popupShadow = mkShadow {
    softness = 12;
    spread = 3;
    y = 6;
  }; # MD3 L5 — floating windows, panels, popups
...
in
{
  # Session autostart for apps that live in the system tray. These can't be niri
  # spawn-at-startup entries: Electron asks the bus once, at tray-creation time,
  ...
  # hosts that watcher and only claims the name a few seconds in, well after niri has
  # spawned everything, so both apps came up permanently invisible: running, hidden by
  # --silent/--start-minimized, with no tray icon to restore them from.
  ...
  # respawn it, matching the old spawn-at-startup behaviour.
  #
  ...
  programs.niri.settings = {
    ...
    # Only apps that need nothing from graphical-session.target belong here: niri
    # runs spawn-at-startup during its own startup and raises that target
    # afterwards, so a spawn can never be ordered after a session service. Apps
    # that need the tray are user units instead (systemd.user.services, above).
    spawn-at-startup = [
      ...
      # Steam is fine as a spawn: libayatana-appindicator watches NameOwnerChanged
      # and re-registers its tray icon whenever the watcher shows up.
      {
        command = [
          "steam"
          "-silent"
