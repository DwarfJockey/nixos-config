...
- `modules/home-manager/{shell,editor,desktop,framework-palette}.nix` — Home Manager modules imported by `home/robert.nix`. `framework-palette.nix` is the single source of truth for colors (consumed by `desktop.nix`).
