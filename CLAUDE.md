...
**Key flake inputs:** nixpkgs (unstable), nixos-hardware, impermanence, home-manager, stylix, nixvim, niri, dms, dms-plugin-registry, zen-browser, firefox-addons, agenix.
...
**Wallpaper:** A single gradient is generated by `stylix.image` in `modules/nixos/theming.nix` using `imagemagick` against the base16 palette. Home Manager symlinks that store path into `~/Pictures/Wallpapers/default.png`, which DMS reads as its wallpaper. Cycling is disabled. Generation only runs during `nixos-rebuild`, not on every boot.
