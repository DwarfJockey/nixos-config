...
**Theming:** Stylix (`modules/nixos/theming.nix`) owns system-wide theming from a single inline base16 scheme ("Over-Engineered Dark", `polarity = "dark"`) — colors, fonts, cursor, icons, and window opacity. With `autoEnable` on it themes every native target (GTK, Qt, Ghostty, Zen, …), and Home Manager's targets inherit the system scheme. Targets with no Stylix support are bridged manually in `desktop.nix`: **Noctalia** (custom palette + opacity), **niri** (target-driven, only border width overridden), and **shadows** (split between niri and Noctalia by surface type). To turn off a misbehaving target: `stylix.targets.<name>.enable = false`. **Full detail — the Noctalia bridge, niri overrides, and the shadow split — is in `agent_docs/theming.md`.**
